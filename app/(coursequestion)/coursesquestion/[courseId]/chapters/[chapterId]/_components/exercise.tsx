  ...
  return (
    <div className=" px-4 py-2 flex flex-col gap-y-4">
      {questions?.map((question, index) => {
        return (
          <div
            className="  rounded-sm flex flex-row items-center justify-between  gap-y-4 bg-slate-200 p-3 "
            key={index}
          >
            <div className="flex items-center justify-between">
              <span className=" line-clamp-1">
                {" "}
                Question {index + 1}: <Link href={`/courses/${courseId}/chapters/${chapterId}/questions/${question?._id}`} className=" text-blue-600 hover:text-blue-800 hover:underline ">{question.title}</Link>
              </span>
            ...
            <div  className=" font-bold"> <ArrowRight></ArrowRight></div>
          </div>
        );
      })}
    </div>
