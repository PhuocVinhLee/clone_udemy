...
export async function POST(
  ...
  try {
    ...
    const questionEndPosition = await QuestionsChapter.findOne({ chapterId: params.chapterId })
    .sort({ position: -1 })
    .exec();
  const position = questionEndPosition ? questionEndPosition.position + 1 : 0;
  const questionData = {
    title,
    ...
    const newQuestion = await QuestionsChapter.create(questionData); // { userId: 1233; title: test;}
