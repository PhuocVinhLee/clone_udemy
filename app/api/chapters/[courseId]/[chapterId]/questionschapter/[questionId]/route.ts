...
export async function PATCH(
  ...
  try {
    ...
    const questionToUpdate = await QuestionsChapter.findById(questionId);

    if (!questionToUpdate || questionToUpdate?.userId.toHexString() !== user._id) {
      throw new Error("Unauthorized or Question Chapter not found");
    }
    ...
    const QuestionUpdated = await QuestionsChapter.findByIdAndUpdate(
      questionId,
      dataQuestion,
      {
        new: false,
